...
def help():
    print("Usage: \
        ...
        \n\tpython3 grabberTelegram.py -u (or --update) <channel_file_name>\
        \nADD 1 CHANNEL:\
        \n\tpython3 grabberTelegram.py --add-channel <app_id> <api_hash> \
        \nPRINT all FOLLOWED CHANNELS:\
        \n\tpython3 grabberTelegram.py --print-followed <app_id> <api_hash>\
        \n PRINT all CHANNELS in DB:\
        \n\tpython3 grabberTelegram.py --print-database\
        \nDELETE all CHANNELS from DB:\
        \n\t python3 grabberTelegram.py --delete-channels\
...
def add_channel(channel):
    try:
        c.execute("INSERT INTO channel (name) VALUES (?)", (channel,))
        conn.commit()
    except:
...
def initialize(id, hash):
    ...
    print("Channels to listen: ", [d.title for d in channels])
    # filter channels
...
if __name__ == "__main__":
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS channel (name text PRIMARY KEY)''')
